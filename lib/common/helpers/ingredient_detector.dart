...
import 'package:smarthealthy/common/constants/detect_model.dart';
import 'package:smarthealthy/data/models/recognition.dart';
...
class IngredientDetector {
  static IngredientDetector? _instance;
  ...
  static Future<IngredientDetector> getInstance() async {
    if (_instance == null) {
      ...
      _instance = IngredientDetector._(
  ...
  List<Recognition> predict(imageLib.Image image) {
    ...
    for (int i = 0; i < amountOfGridCells.length; i += (5 + numClasses)) {
      if (amountOfGridCells[i + 4] < objectScore) continue;
      // Check class score
    ...
    developer.log(recognitions.length.toString());
